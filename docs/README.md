...
The project has a custom mdbook preprocessor (`crates/docs_preprocessor`).

If for some reason you need to bypass the docs preprocessor, you can comment out `[preprocessor.gram_docs_preprocessor]
` from the `book.toml`.

## Images and videos

To add images or videos to the docs, upload them to another location and then link out to them from the docs.
...
This will render a human-readable version of the action name, e.g., "gram: open settings", and enables things like additional context on hover, etc.

### Creating New Templates

Templates are just functions that modify the source of the docs pages (usually with a regex match & replace). See how the actions and keybindings are templated in `crates/docs_preprocessor/src/main.rs` for reference on how to create new templates.
