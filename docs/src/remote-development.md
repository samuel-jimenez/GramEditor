...
Remote Development allows you to code at the speed of thought, even when your codebase is not on your local machine. You use Tehanu locally so the UI is immediately responsive, but offload heavy computation to the development server so that you can work effectively.

## Overview

Remote development requires two computers, your local machine that runs the Tehanu UI and the remote server which runs a Tehanu headless server. The two communicate over SSH, so you will need to be able to SSH from your local machine into the remote server to use this feature.

![Architectural overview of Tehanu Remote Development](https://tehanu.liten.app/img/remote-development/diagram.png)

On your local machine, Tehanu runs its UI, talks to language models, uses Tree-sitter to parse and syntax-highlight code, and store unsaved changes and recent projects. The source code, language servers, tasks, and the terminal all run on the remote server.

> **Note:** The original version of remote development sent traffic via Tehanu's servers. As of Tehanu v0.157 you can no-longer use that mode.

## Setup

1. Download and install the latest [Tehanu](https://tehanu.liten.app/releases). You need at least Tehanu v0.159.
...
1. Your local machine will attempt to connect to the remote server using the `ssh` binary on your path. Assuming the connection is successful, Tehanu will download the server on the remote host and start it.
1. Once the Tehanu server is running, you will be prompted to choose a path to open on the remote server.
   > **Note:** Tehanu does not currently handle opening very large directories (for example, `/` or `~` that may have >100,000 files) very well. We are working on improving this, but suggest in the meantime opening only specific projects, or subfolders of very large mono-repos.
...
The remote machine must be able to run Tehanu's server. The following platforms should work, though note that we have not exhaustively tested every Linux distribution:
...
The list of remote servers is stored in your settings file {#kb tehanu::OpenSettings}. You can edit this list using the Remote Projects dialog {#kb projects::OpenRemote}, which provides some robustness - for example it checks that the connection can be established before writing it to the settings file.
...
Tehanu shells out to the `ssh` on your path, and so it will inherit any configuration you have in `~/.ssh/config` for the given host. That said, if you need to override anything you can configure the following additional options on each connection:
...
There are two additional Tehanu-specific options per connection, `upload_binary_over_ssh` and `nickname`:
  ...
  "ssh_connections": [
    {
      "host": "192.168.1.10",
      "projects": [{ "paths": ["~/code/zed/zed"] }],
      // by default Tehanu will download the server binary from the internet on the remote.
      ...
      // Shown in the Tehanu UI to help distinguish multiple hosts.
      "nickname": "lil-linux"
    }
  ]
}
...
Tehanu on Windows supports SSH remoting and will prompt for credentials when needed.
...
Tehanu supports opening folders inside of WSL natively on Windows.
...
- The local Tehanu settings (in `~/.tehanu/settings.json` on macOS or `~/.config/tehanu/settings.json` on Linux) on your local machine.
- The server Tehanu settings (in the same place) on the remote server.
- The project settings (in `.zed/settings.json` or `.editorconfig` of your project)

Both the local Tehanu and the server Tehanu read the project settings, but they are not aware of the other's main `settings.json`.
...
See the [proxy documentation](./configuring-tehanu.md#network-proxy) for supported proxy types and additional configuration options.

## Initializing the remote server

Once you provide the SSH options, Tehanu shells out to `ssh` on your local machine to create a ControlMaster connection with the options you provide.

Any prompts that SSH needs will be shown in the UI, so you can verify host keys, type key passwords, etc.

Once the master connection is established, Tehanu will check to see if the remote server binary is present in `~/.tehanu_server` on the remote, and that its version matches the current version of Tehanu that you're using.

If it is not there or the version mismatches, Tehanu will try to download the latest version. By default, it will download from `https://tehanu.liten.app` directly, but if you set: `{"upload_binary_over_ssh":true}` in your settings for that server, it will download the binary to your local machine and then upload it to the remote server.

If you'd like to maintain the server binary yourself you can. You can either download our prebuilt versions from [GitHub](https://github.com/zed-industries/zed/releases), or [build your own](https://tehanu.liten.app/docs/development) with `cargo build -p remote_server --release`. If you do this, you must upload it to `~/.tehanu_server/zed-remote-server-{RELEASE_CHANNEL}-{VERSION}` on the server, for example `~/.tehanu_server/zed-remote-server-stable-0.181.6`. The version must exactly match the version of Tehanu itself you are using.

## Maintaining the SSH connection

Once the server is initialized. Tehanu will create new SSH connections (reusing the existing ControlMaster) to run the remote development server.
...
In the case that reconnecting fails, the daemon will not be re-used. That said, unsaved changes are by default persisted locally, so that you do not lose work. You can always reconnect to the project at a later date and Tehanu will restore unsaved changes.

If you are struggling with connection issues, you should be able to see more information in the Tehanu log `cmd-shift-p Open Log`. If you are seeing things that are unexpected, please file a [GitHub issue](https://github.com/zed-industries/zed/issues/new) or reach out in the #remoting-feedback channel in the [Tehanu Discord](https://tehanu.liten.app/community-links).

## Supported SSH Options

Under the hood, Tehanu shells out to the `ssh` binary to connect to the remote server. We create one SSH control master per project, and then use that to multiplex SSH connections for the Tehanu protocol itself, any terminals you open and tasks you run. We read settings from your SSH config file, but if you want to specify additional options to the SSH control master you can configure Tehanu to set them.
...
Note that we deliberately disallow some options (for example `-t` or `-T`) that Tehanu will set for you.
...
Please join the #remoting-feedback channel in the [Tehanu Discord](https://tehanu.liten.app/community-links).
