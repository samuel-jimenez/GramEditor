...
Remote Development allows you to code at the speed of thought, even when your codebase is not on your local machine. You use Gram locally so the UI is immediately responsive, but offload heavy computation to the development server so that you can work effectively.

## Overview

Remote development requires two computers, your local machine that runs the Gram UI and the remote server which runs a Gram headless server. The two communicate over SSH, so you will need to be able to SSH from your local machine into the remote server to use this feature.

![Architectural overview of Gram Remote Development](https://gram.liten.app/img/remote-development/diagram.png)

On your local machine, Gram runs its UI, talks to language models, uses Tree-sitter to parse and syntax-highlight code, and store unsaved changes and recent projects. The source code, language servers, tasks, and the terminal all run on the remote server.

> **Note:** The original version of remote development sent traffic via Gram's servers. As of Gram v0.157 you can no-longer use that mode.

## Setup

1. Download and install the latest [Gram](https://gram.liten.app/releases). You need at least Gram v0.159.
...
1. Your local machine will attempt to connect to the remote server using the `ssh` binary on your path. Assuming the connection is successful, Gram will download the server on the remote host and start it.
1. Once the Gram server is running, you will be prompted to choose a path to open on the remote server.
   > **Note:** Gram does not currently handle opening very large directories (for example, `/` or `~` that may have >100,000 files) very well. We are working on improving this, but suggest in the meantime opening only specific projects, or subfolders of very large mono-repos.
...
The remote machine must be able to run Gram's server. The following platforms should work, though note that we have not exhaustively tested every Linux distribution:
...
The list of remote servers is stored in your settings file {#kb gram::OpenSettings}. You can edit this list using the Remote Projects dialog {#kb projects::OpenRemote}, which provides some robustness - for example it checks that the connection can be established before writing it to the settings file.
...
Gram shells out to the `ssh` on your path, and so it will inherit any configuration you have in `~/.ssh/config` for the given host. That said, if you need to override anything you can configure the following additional options on each connection:
...
There are two additional Gram-specific options per connection, `upload_binary_over_ssh` and `nickname`:
  ...
  "ssh_connections": [
    {
      "host": "192.168.1.10",
      "projects": [{ "paths": ["~/code/zed/zed"] }],
      // by default Gram will download the server binary from the internet on the remote.
      ...
      // Shown in the Gram UI to help distinguish multiple hosts.
      "nickname": "lil-linux"
    }
  ]
}
...
If it is not there or the version mismatches, Gram will try to download the latest version. By default, it will download from `https://gram.liten.app` directly, but if you set: `{"upload_binary_over_ssh":true}` in your settings for that server, it will download the binary to your local machine and then upload it to the remote server.

If you'd like to maintain the server binary yourself you can. You can either download our prebuilt versions from [GitHub](https://github.com/zed-industries/zed/releases), or [build your own](https://gram.liten.app/docs/development) with `cargo build -p remote_server --release`. If you do this, you must upload it to `~/.gram_server/zed-remote-server-{RELEASE_CHANNEL}-{VERSION}` on the server, for example `~/.gram_server/zed-remote-server-stable-0.181.6`. The version must exactly match the version of Gram itself you are using.
...
If you are struggling with connection issues, you should be able to see more information in the Gram log `cmd-shift-p Open Log`. If you are seeing things that are unexpected, please file a [GitHub issue](https://github.com/zed-industries/zed/issues/new) or reach out in the #remoting-feedback channel in the [Gram Discord](https://gram.liten.app/community-links).
...
Please join the #remoting-feedback channel in the [Gram Discord](https://gram.liten.app/community-links).
