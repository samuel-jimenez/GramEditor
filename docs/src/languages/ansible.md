...
Support for Ansible in Tehanu is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/zed-ansible).
...
To avoid mishandling non-Ansible YAML files, the Ansible Language is not associated with any file extensions by default. To change this behavior you can add a `"file_types"` section to Tehanu settings inside your project (`.zed/settings.json`) or your Tehanu user settings (`~/.config/tehanu/settings.json`) to match your folder/naming conventions. For example:
...
- Or configure the yaml language server settings to set this schema for all your inventory files, that match your inventory pattern, under your Tehanu settings ([ref](https://tehanu.liten.app/docs/languages/yaml)):
...
When desired, any of the above default settings can be overridden under the `"lsp"` section of your Tehanu settings file. For example:

```json [settings]
"lsp": {
  // Note, the Tehanu Ansible extension prefixes all settings with `ansible`
