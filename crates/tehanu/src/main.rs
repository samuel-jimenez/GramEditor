...
use crate::tehanu::{OpenRequestKind, eager_load_active_theme_and_icon_theme};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Tehanu failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Tehanu failed to open a window: {e:?}. See https://tehanu.liten.app/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Tehanu failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://tehanu.liten.app/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
pub fn main() {
    ...
    match util::get_zed_cli_path() {
        ...
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("TEHANU_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    let app_commit_sha = option_env!("TEHANU_COMMIT_SHA")
    ...
    if args.system_specs {
        ...
        println!("Tehanu System Specs (from CLI):\n{}", system_specs);
        return;
    }
    ...
    let failed_single_instance_check = if *app_env_vars::TEHANU_STATELESS
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            crate::tehanu::listen_for_cli_connections(open_listener.clone()).is_err()
        }

        #[cfg(target_os = "windows")]
        {
            !crate::tehanu::windows_only_instance::handle_single_instance(
                open_listener.clone(),
                &args,
            )
        }
    ...
    app.run(move |cx| {
        ...
        crate::tehanu::component_preview::init(app_state.clone(), cx);
    ...
    if let Some(kind) = request.kind {
        match kind {
            ...
            OpenRequestKind::Setting { setting_path } => {
                // tehanu://settings/languages/$(language)/tab_size  - DONT SUPPORT
                // tehanu://settings/languages/Rust/tab_size  - SUPPORT
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        ...
                        Some(setting_path) => window.dispatch_action(
                            Box::new(app_actions::OpenSettingsAt { path: setting_path }),
                            cx,
                        ),
                    })
                })
...
#[derive(Parser, Debug)]
#[command(name = "tehanu", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `tehanu://` scheme, or relative to <https://liten.app>.
    ...
    /// On macOS, the default is `~/Library/Application Support/Tehanu`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zed`.
    /// On Windows, the default is `%LOCALAPPDATA%\Tehanu`.
    ...
    /// prevents Tehanu from starting, so you can't run `tehanu: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,
    ...
    /// Used for recording minidumps on crashes by having Tehanu run a separate
