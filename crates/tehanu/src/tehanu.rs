...
use app_actions::{OpenBrowser, OpenDocs, OpenServerSettings, OpenSettingsFile, OpenTehanuUrl, Quit};
...
fn unstable_version_notification(cx: &mut App) {
    ...
    show_app_notification(NotificationId::unique::<WindowsNightly>(), cx, |cx| {
        cx.new(|cx| {
            MessageNotification::new("You're using an unstable version of Tehanu (Nightly)", cx)
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://tehanu.liten.app/docs/linux
            "#},
            e
        );
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://tehanu.liten.app/docs/linux#could-not-start-inotify");
                    cx.quit();
                })
                .ok();
            }
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: https://tehanu.liten.app/docs/windows
            "#},
            e
        );
        let prompt = window.prompt(
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url("https://tehanu.liten.app/docs/windows");
                    cx.quit()
                })
                .ok();
            }
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("TEHANU_ALLOW_EMULATED_GPU").is_err() {
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                "https://tehanu.liten.app/docs/windows",
                "https://tehanu.liten.app/docs/windows",
            )
        } else {
            (
                ...
                "https://tehanu.liten.app/docs/linux#zed-fails-to-open-windows",
            )
        };
        let message = format!(
            db::indoc! {r#"
            Tehanu uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set TEHANU_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenTehanuUrl, _, cx| {
        ...
        .register_action(|_, _: &install_cli::RegisterUriScheme, window, cx| {
            ...
            .detach_and_prompt_err(
                "Error registering tehanu:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        let original_settings = app_state
            .fs
            .load(Path::new("/root/.tehanu/settings.json"))
            .await
            .unwrap();
