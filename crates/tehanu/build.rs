...
fn main() {
    if cfg!(target_os = "macos") {
        ...
        // Weakly link ReplayKit to ensure Tehanu can be used on macOS 10.15+.
        ...
        && output.status.success()
    {
            ...
            && build_profile == "release"
        {
            ...
            println!("cargo::warning=Info: using '{git_sha}' hash for TEHANU_COMMIT_SHA env var");
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(explicit_rc_toolkit_path) = std::env::var("TEHANU_RC_TOOLKIT_PATH").ok() {
            res.set_toolkit_path(explicit_rc_toolkit_path.as_str());
        }
        res.set_icon(icon.to_str().unwrap());
        res.set("FileDescription", "Tehanu");
        res.set("ProductName", "Tehanu");
