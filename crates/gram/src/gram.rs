...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("GRAM_ALLOW_EMULATED_GPU").is_err() {
        ...
        let message = format!(
            db::indoc! {r#"
            Gram uses {} for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: {}
            Set GRAM_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenGramUrl, _, cx| {
...
mod tests {
    ...
    async fn test_opening_project_settings_when_excluded(cx: &mut gpui::TestAppContext) {
        ...
        eprintln!("Added .gram to file_scan_exclusions in settings");
        ...
        assert!(
            !has_entry,
            "Test precondition failed: .gram directory should be excluded but was found in worktree"
        );
