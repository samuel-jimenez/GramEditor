...
pub fn main() {
    ...
    match util::get_gram_cli_path() {
        ...
        Err(err) => {
            eprintln!("Error: {}", err);
            if std::option_env!("GRAM_BUNDLE").is_some() {
                process::exit(1);
            }
        }
    }
    ...
    app.background_executor()
        .spawn(crashes::init(InitCrashHandler {
            session_id,
            gram_version: app_version.to_string(),
        ...
        || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev
    {
        ...
        #[cfg(target_os = "windows")]
        {
            !crate::gram::windows_only_instance::handle_single_instance(
                open_listener.clone(),
                &args,
            )
        }
    ...
    app.run(move |cx| {
        ...
        crate::gram::component_preview::init(app_state.clone(), cx);
...
#[derive(Parser, Debug)]
#[command(name = "gram", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    /// Used for recording minidumps on crashes by having Gram run a separate
